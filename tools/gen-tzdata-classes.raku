...
sub MAIN($tzdata-file, $output-dir) {
    ...
    if $parsed {
        ...
        for @zones -> $zone {
            ...
            $name ~~ s:g/\+/_plus_/;
            $name ~~ s:g/\-/_minus_/;
            my $dir = ($output-dir ~ $name ~ ".rakumod").IO.dirname;
            while !($dir.IO ~~ :d) {
            ...
            my $fh = open($output-dir ~ $name ~ ".rakumod", :w);
            my $classname = $name;
            $classname ~~ s:g/\//::/;
            ...
            $fh.say('has %.rules = ( ');
            for @rules -> $rule {
                $fh.say(" '$rule' => " ~ %ruledata{$rule}.raku ~ ",");
            }
            $fh.say(");");
            $fh.say('has @.zonedata = ' ~ @zonedata.raku ~ ';');
            $fh.close();
        }
        ...
        for @links -> $link {
            ...
            my @dirs_to_make;
            my $dir = ($output-dir ~ $old-tz ~ ".rakumod").IO.dirname;
            while !($dir.IO ~~ :d) {
                @dirs_to_make.unshift($dir);
                $dir = $dir.IO.parent;
            }
            ...
            my $fh = open($output-dir ~ $old-tz ~ ".rakumod", :w);
